...
public class TalkManager : MonoBehaviour
{
    ...
    void GenerateData()
    {
        ...
        talkData.Add(50 + 3000, new string[] { "분위기가 달라졌어.:1"});
        talkData.Add(60 + 3000, new string[] { "저것이 아까 그 아이가 말한 위험한 존재인 건가.:1", "대화가 통하지 않는다고 했었지만... 혹시 모르니까 한 번 다가가보자.:1" });

        talkData.Add(70 + 12000, new string[] { "경찰차다. 그러나 사람은 없는 것 같다.:1" });
        talkData.Add(70 + 13000, new string[] { "평범한 자동차이다.:1" });
        talkData.Add(70 + 14000, new string[] { "뭐지? 이 차는 뭔가 익숙해.:1", "그러고보니 얼마 전에 차를 타고 가족들과 함께 바다로 여행을 갔었지.:1", "그때 보았던 바다가 정말 예뻤었는데...:1" }); //이름 일단 제로로 (제로가 말한 것처럼 나와야 하니까)

        talkData.Add(73 + 3000, new string[] { "우와! 여기는 사람이 많잖아?:1" });
        ...
        portraitData.Add(3000 + 1, portraitArr[1]); //제로 > 제로
        portraitData.Add(2000 + 0, portraitArr[0]); //고양이 > none
        portraitData.Add(4000 + 0, portraitArr[0]); //누나 > none
